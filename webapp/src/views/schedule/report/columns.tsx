...
import { report } from "@/api/schedule"
...
export function useColumns() {
        ...
        addDialog({
            ...
            closeCallBack: async ({ args }) => {
                if (args?.command === "cancel") {
                    ...
                    const res = await report.delete(row.name);
                    if (res.code === 200) {
    ...
    async function handleStart(param: any, row: undefined) {
        if (row.status === 1) {
            const res = await report.stop(row.name)
            if (res.code === 200) {
                fetchData().then(r => {
            ...
            const res = await report.start(row.name)
            if (res.code === 200) {
                fetchData().then(r => {
