...
---
package.path = "./plugin/2018F214-33/?.lua;"
...
function AnalysisRx(sAddr, rxBufCnt)
    ...
    if ctrlCode == 0x81 then
        ...
        if cmd == "901F" or cmd == "1F90" then
            ...
            -- 前3字节是FE唤醒符，已移除
            -- 根据实际日志数据分析，流量数据在不同位置，需要查找0x2C单位标记
            
            -- 安全计数器防止无限循环
            local dataStart = 16  -- 默认从16字节开始搜索
            local unitMarkerFound = false
            local data = {}
            
            -- 查找0x2C单位标记
            for i = 16, math.min(30, #rxBuf) do
                if rxBuf[i] == 0x2C then
                    unitMarkerFound = true
                    -- 单位标记前的4字节是数据
                    for j = i-4, i-1 do
                        if j >= 1 and j < i then
                            table.insert(data, rxBuf[j])
                        end
                    end
                    break
                end
            ...
            -- 如果没找到0x2C单位标记，尝试固定位置取值
            if not unitMarkerFound then
                print("警告: 未找到0x2C单位标记，使用默认位置")
                for i = 16, 19 do
                    if i <= #rxBuf then
            ...
            if #data > 0 then
                -- 根据CJ/T 188-2004标准，水表数据使用BCD码编码
                -- 每个字节表示两位十进制数字
                local bcdStr = ""
                for i = #data, 1, -1 do
                    bcdStr = bcdStr .. string.format("%02X", data[i])
                end
                
                -- BCD码解析（例如：00001900表示1900）
                -- BCD码直接表示十进制数字，所以可以直接转换
                print("解析BCD流量值:", bcdStr)
                local dev_flow = tonumber(bcdStr, 10)
                print("实际流量:", dev_flow)
    ...
    elseif ctrlCode == 0xA5 then
        ...
        if cmd == "A017" or cmd == "17A0" then
            -- 确保索引在有效范围内
            if 15 + 1 > #rxBuf - 1 then
                print("警告: 数据包长度不足，无法解析阀门状态")
                rxBuf = {}
                return {Status = "1", Variable = {}}
            end
            
            local status = string.format("%02X%02X", rxBuf[15], rxBuf[16] or 0xFF)
            rxBuf = {}
...
function GenerateGetRealVariables(sAddr, step)
    print("2018F214-33 ver 1.0", sAddr, step)
    if (step == 0) then
